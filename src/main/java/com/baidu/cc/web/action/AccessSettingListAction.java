...
import com.baidu.cc.web.action.AccessSettingListActionBase.DataResult;
import com.baidu.cc.web.action.AccessSettingListActionBase.ReqParam;
 ...
 * @author BJF
 */
public class AccessSettingListAction extends
        AccessSettingListActionBase<ReqParam, DataResult> {
    ...
    protected DataResult doExecuteService(ReqParam reqParam) {
        Project project = projectService.findById(reqParam.getProjectId());
        List<AccessSetting> accessSettingList = accessSettingService
                .findByRefIdAndType(reqParam.getProjectId(),
                        SysUtils.ACCESS_SETTING_TYPE_PROJECT);
        getData().setAccessList(accessSettingList);
        getData().setProject(project);
