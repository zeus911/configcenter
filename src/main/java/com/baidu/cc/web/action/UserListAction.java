...
import com.baidu.cc.web.action.UserListActionBase.DataResult;
import com.baidu.cc.web.action.UserListActionBase.ReqParam;
 ...
 * @author BJF
 */
public class UserListAction extends UserListActionBase<ReqParam, DataResult> {
    ...
    protected DataResult doExecuteService(ReqParam reqParam) {
        ...
        // if search name is active
        String name = reqParam.getName();
        List<User> users;
        if (StringUtils.isNotBlank(name)) {
            users = (List<User>) executeService(UserService.class,
                    "findByNameFilter", name);
        } else {
            users = (List<User>) executeService(UserService.class, "findAll",
                    null);
        }

        getData().setList(users);
