...
import com.baidu.cc.web.action.ConfigItemListActionBase.DataResult;
import com.baidu.cc.web.action.ConfigItemListActionBase.ReqParam;
 ...
 * @author BJF
 */
public class ConfigItemListAction extends
        ConfigItemListActionBase<ReqParam, DataResult> {
    ...
    protected DataResult doExecuteService(ReqParam reqParam) {
        ...
        ConfigGroup group = configGroupService.findById(groupId);
        Version version = versionService.findById(group.getVersionId());
        Environment env = environmentService.findById(version
                .getEnvironmentId());
        Project project = projectService.findById(env.getProjectId());

        boolean resolveReference = false;
        List<ConfigItem> configItems = configItemService.findByGroupId(groupId,
                resolveReference);
        DataResult result = new DataResult();
        result.setConfigItemList(configItems);
        result.setEnv(env);
        result.setConfigGroup(group);
