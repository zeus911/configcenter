...
import com.baidu.cc.web.action.ConfigGroupListActionBase.DataResult;
import com.baidu.cc.web.action.ConfigGroupListActionBase.ReqParam;
 ...
 * @author BJF
 */
public class ConfigGroupListAction extends
        ConfigGroupListActionBase<ReqParam, DataResult> {
    ...
    protected DataResult doExecuteService(ReqParam reqParam) {
        List<ConfigGroup> configGroupList = configGroupService
                .findByVersionId(reqParam.getVersionId());
        Version version = versionService.findById(reqParam.getVersionId());
        Environment env = environmentService.findById(version
                .getEnvironmentId());
        Project project = projectService.findById(env.getProjectId());
        DataResult result = new DataResult();
        result.setConfigGroupList(configGroupList);
