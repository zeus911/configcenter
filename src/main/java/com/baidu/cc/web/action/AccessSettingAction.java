...
import com.baidu.cc.web.action.AccessSettingActionBase.ReqParam;
 ...
 * @author BJF
 */
public class AccessSettingAction extends
        AccessSettingActionBase<ReqParam, DataResult> {
    ...
    protected DataResult doExecuteService(ReqParam reqParam) {
        ...
        if ("del".equals(reqParam.getOperationType())) {
            accessSettingService.delete(reqParam.getUserId(),
                    reqParam.getProjectId(),
                    SysUtils.ACCESS_SETTING_TYPE_PROJECT);
        } else if ("add".equals(reqParam.getOperationType())) {
            AccessSetting accessSetting = new AccessSetting();
            accessSetting.setUserId(reqParam.getUserId());
            accessSetting.setRefId(reqParam.getProjectId());
            accessSetting.setType(Byte
                    .valueOf(SysUtils.ACCESS_SETTING_TYPE_PROJECT + ""));
