...
import com.baidu.cc.web.action.EnvironmentSaveActionBase.ReqParam;
 ...
 * @author BJF
 */
public class EnvironmentSaveAction extends
        EnvironmentSaveActionBase<ReqParam, DataResult> {
    ...
    protected Map<String, String> doValidate(ReqParam reqParam) {
        ...
        Environment env = reqParam.getEnv();
        if (env == null) {
            result.put("envNull", "请输入所有必填项");
        } else {
            if (StringUtils.isBlank(env.getName())
                    || env.getName().length() > 50) {
                result.put("nameLengthError", "名称必须是1到50长度的字符串");
            }
            if (StringUtils.isNotBlank(env.getMemo())
                    && env.getMemo().length() > 255) {
    ...
    protected DataResult doExecuteService(ReqParam reqParam) {
        ...
        if (env.getId() == null) {
            ...
            environmentService.updateEntitySelective(env);
        }
